...
class SupervisorAgent:
    ...
    def action(self):
        ...
        if "nein" in thought:
            ...
            validation_result = self.validation_agent.run_with_prompt(improved_prompt)
            self.save_intermediate_result("validation", validation_result)
        ...
        if check_result in ["nicht_nachvollziehbar", "unklar"]:
            ...
            return "Warte auf Benutzerentscheidung"

        # Setze Workflow fort
        return self.continue_after_user_approval()

    def continue_after_user_approval(self):
    ...
    def ask_user(self, missing_info):
        return input(f"Supervisor fragt: '{missing_info}'. Bitte geben Sie die fehlende Information ein: ")
